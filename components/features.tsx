...
export default function Features({ children, color, colorDark }: FeatureProp) {
  ...
  return (
    ...
    >
      <div className="mt-[12.8rem] mb-16 w-full md:mt-[25.2rem] md:mb-[12.8rem]">{children}</div>
    </section>
  );
}
  ...
  return (
    <>
      <div className="relative before:absolute before:inset-0 before:bg-[radial-gradient(ellipse_50%_50%_at_center,rgba(var(--feature-color),0.1),transparent)]">
        <Container
          className={classNames('max-w-[90%] text-center', imageSize === 'small' ? 'w-[78rem]' : 'w-[102.4rem]')}
        >
      ...
      <Container className="w-[78rem] max-w-[90%] text-center">
        <p className="mx-auto my-16 text-2xl leading-tight text-white md:w-[80%] md:text-4xl">{text}</p>
      ...
      <div className="grid w-full grid-cols-1 gap-6 md:grid-cols-2">
        {features.map(({ title, text, image, imageClassName }) => (
          ...
          >
            ...
            <img className={classNames('absolute max-w-none', imageClassName)} src={image} />
          </div>
        ))}
      </div>
    </Container>
